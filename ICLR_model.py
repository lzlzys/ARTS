...
class SASRecModel(nn.Module):
    ...
    def add_position_embedding(self, sequence, A=None):
        ...
        if A != None:#替换全局特征
            mask = torch.eq(sequence, 0)
            pad_count = torch.sum(mask, dim=1)
            for i in range(sequence.size(0)):
                if pad_count[i] == 0:  # 如果没有pad，则加上cls_token
                    left = A[i,:,:].unsqueeze(0)
                    right = item_embeddings[i, :].unsqueeze(0)
                    new_enc_outputs = torch.cat((left, right), dim=1)
                    item_embeddings[i] = new_enc_outputs[:,0:-1,:]
                else:  # 如果有pad，则将序列拆成两个序列，一个是pad，一个是实际的交互序列，然后在交互序列的最前面加上cls_token
                    left = item_embeddings[i][:pad_count[i]]
                    # left = self.src_emb(torch.LongTensor(left.cpu()).cuda())
                    right = item_embeddings[i][pad_count[i]:]
                    ...
                    new_enc_outputs = torch.cat((left.unsqueeze(0), A[i,:,:].unsqueeze(0), right.unsqueeze(0)), dim=1)
                    item_embeddings[i] = new_enc_outputs[:,0:-1,:]

            # item_embeddings[:,0,:] = A[:,0,:]
    ...
    def forward2(self,input_ids, A):
        A = A.unsqueeze(1)#形成添加A后的MASK矩阵
        a = None
        for i in input_ids:
            length = 0
            for j in i:
            ...
            if length != 0:
                b = torch.rand(1, input_ids.size(1) - length + 1).cuda()
                c = torch.zeros(1, length - 1).cuda()
                d = torch.cat((c, b), dim=1).cuda()
            else:
                d = i.unsqueeze(0).cuda()
            if a == None:
                a = d.cuda()
            else:
        ...
        sequence_emb = self.add_position_embedding(input_ids, A)



        item_encoded_layers = self.item_encoder(sequence_emb, extended_attention_mask, output_all_encoded_layers=True)
        sequence_output = item_encoded_layers[-1]
        # sequence_output = torch.cat((A, sequence_output), dim=1)
