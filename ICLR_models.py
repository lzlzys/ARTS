...
from ICLR_module import Encoder, LayerNorm
...
class SASRecModel(nn.Module):
    ...
    def add_position_embedding(self, sequence):
        ...
        # for i in user_id:
        #
        #     torch.save(item_embeddings[i,:], '/home/LiuZunLong/python_code/ICLR/new_item_embedding/{}_new_item_embedding'.format(i + 1))
        ...
        return sequence_emb, item_embeddings

    # model same as SASRec
    def forward(self,input_ids, train):


        # if train == True:
        #
        #     size1 = input_ids.size(0)
        #     size2 = input_ids.size(1)
        #     flag_embedding = torch.randint(low=0, high=1000, size=(size1, size2)).long()
        #     flag_embedding = flag_embedding[:, 0]
        #     flag_embedding = flag_embedding.unsqueeze(1)
        #     flag_embedding = flag_embedding.cuda()
        #     input_ids = input_ids[:, 0:99]#input_ids 255*100
        #     input_ids1 = input_ids
        #     input_ids1 = input_ids1.cuda()
        #     input_ids =torch.cat((flag_embedding, input_ids), 1)
        ...
        #     flag_embedding = flag_embedding.cuda()
        #     input_ids = input_ids[:, :]#input_ids 255*100
        #     input_ids1 = input_ids
        #     input_ids1 = input_ids1.cuda()
        #     input_ids =torch.cat((flag_embedding, input_ids), 1)
        #
        #     sequence_emb, item_embeddings = self.add_position_embedding(input_ids)
        #
