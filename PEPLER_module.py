...
class UIPrompt:
    ...
    def forward(self, A, B, text, mask=None, ignore_index=-100):#B 100*768

        device = text.device
        batch_size = text.size(0)
        A = A.unsqueeze(0)  # 1*768
        C = A.repeat(B.size(0), 1)  # 20* 768

        # embeddings
        # u_src = self.user_embeddings(user)  # (batch_size, emsize)
        # i_src = self.item_embeddings(item)  # (batch_size, emsize)
        w_src = self.transformer.wte(text)  # (batch_size, tgt_len, emsize) 100, 25, 768
        # src = torch.cat([u_src.unsqueeze(1), i_src.unsqueeze(1), w_src], 1)  # (batch_size, total_len, emsize)
        # C = C[0:w_src.size(0),:]
        # w_src = w_src[,0,:]
        # w_src = w_src.unsqueeze(0).unsqueeze(1)
        # src = torch.cat((C[0:w_src.size(0),:,:], w_src), 1)
        src = torch.cat((C.unsqueeze(1), B.unsqueeze(1), w_src), 1)
        if mask is None:
            # auto-regressive generation
            return super().forward(inputs_embeds=src)
        else:

            pad_left = torch.ones((C.size(0), self.src_len), dtype=torch.int64).to(device)

            pad_input = torch.cat([pad_left, mask], 1)  # (batch_size, total_len)

            pred_left = torch.full((C.size(0), self.src_len), ignore_index, dtype=torch.int64).to(
    ...
    def forward2(self,user, item, text, mask, ignore_index=-100):#B batch*100*768


        device = user.device
        batch_size = user.size(0)
        # A = A.unsqueeze(1) # 64*1*768
        # C = np.multiply(A, B[0,0,:]).cuda()


        # embeddings
        u_src = self.user_embeddings(user)  # (batch_size, emsize)
        i_src = self.item_embeddings(item)  # (batch_size, emsize)
        w_src = self.transformer.wte(text)  # (batch_size, tgt_len, emsize)64 * 23 * 768
        src = torch.cat([u_src.unsqueeze(1), i_src.unsqueeze(1), w_src], 1)  # (batch_size, total_len, emsize)
    ...
    def forward3(self, A, B, text, mask=None, ignore_index=-100):# A复制20份
        device = text.device
        batch_size = text.size(0)
        A = A.unsqueeze(0) # 1*768
        C = A.repeat(B.size(0), 1)#20* 768
        ...
        if mask is None:
            ...
            pred_left = torch.full((C.size(0), self.src_len), ignore_index, dtype=torch.int64).to(device)  # (batch_size, src_len)
            pred_right = torch.where(mask == 1, text, torch.tensor(ignore_index).to(device))  # replace <pad> with ignore_index
            prediction = torch.cat([pred_left, pred_right], 1)  # (batch_size, total_len)
