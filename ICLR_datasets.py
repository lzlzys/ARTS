...
from ILCR_data_augmentation import Crop, Mask, Reorder, Random
from ICLR_utils import neg_sample, nCr
import copy


class RecWithContrastiveLearningDataset(Dataset):
    def __init__(self, args, user_seq, seq, test_neg_items=None, data_type="train", similarity_model_type="offline"):
        self.args = args
    ...
    def _data_sample_rec_task(self, user_id, items, input_ids, target_pos, answer, seq, tokenizer):
        ...
        n = len(copied_input_ids)
        mean = 0
        std = 1
        noise = np.random.normal(mean, std, size=n).tolist()
        noisy_data = []
        # noisy_data = copied_input_ids + noise
        for i in range(len(copied_input_ids)):
            if copied_input_ids[i] != 1:
                noisy_data.append(copied_input_ids[i] + noise[i])
            else:
                noisy_data.append(copied_input_ids[i])

        self.max_len = 10
        # print('self.max_len = {}'.format(self.max_len))
        pad_len = self.max_len - len(copied_input_ids)
        copied_input_ids = [0] * pad_len + copied_input_ids
        target_pos = [0] * pad_len + target_pos
        target_neg = [0] * pad_len + target_neg
        noisy_data = [0] * pad_len + noisy_data

        noisy_data = noisy_data[-self.max_len :]
        seq = ['1'] * pad_len + seq
        for i in seq:
            words = i.split()
            pad_len = 20 - 2 - len(words)
            # print(type(i))
            if pad_len > 0:
                i = '1 ' * pad_len + i
            seq[j] = i
        ...
        # self.test_neg_items = target_neg
        # copied_input_ids = copied_input_ids[ :self.max_len]
        # target_pos = target_pos[ :self.max_len]
        # target_neg = target_neg[ :self.max_len]
        encoded_inputs = tokenizer(seq[-self.max_len :], padding=True, return_tensors='pt')

        seq = encoded_inputs['input_ids'].contiguous()
        seq = seq[:, : self.max_len ]
        if seq.size(0) != self.max_len:
            new_seq = torch.zeros(self.max_len - seq.size(0), seq.size(1)).to(seq.device)
            seq = torch.cat([seq, new_seq], dim=0)
        mask = encoded_inputs['attention_mask'].contiguous()
        mask = mask[:, : self.max_len]
        if mask.size(0) != self.max_len:
            new_mask = torch.zeros(self.max_len - mask.size(0), mask.size(1)).to(mask.device)
            mask = torch.cat([mask, new_mask], dim=0)
        assert len(copied_input_ids) == self.max_len
        assert len(target_pos) == self.max_len
        assert len(target_neg) == self.max_len
        assert len(noisy_data) == self.max_len
        if self.test_neg_items is not None:
            ...
            cur_rec_tensors = (
                ...
                torch.tensor(seq, dtype=torch.long),
                torch.tensor(mask, dtype=torch.long),
                torch.tensor(noisy_data, dtype=torch.long),
            )

        return cur_rec_tensors
    def _data_sample_rec_task1(self, user_id, items, input_ids, target_pos, answer, seq, tokenizer):
        # make a deep copy to avoid original sequence be modified
        copied_input_ids = copy.deepcopy(input_ids)
        target_neg = []
        seq_set = set(items)
    ...
    def __getitem__(self, index):
        ...
        seq = seq[:len(input_ids)]
        if self.data_type == "train":
            #另加的
            # flag_embedding1 = random.randint(1, 2900)
            # input_ids.insert(0, flag_embedding1)
            #

            cur_rec_tensors = self._data_sample_rec_task(user_id, items, input_ids, target_pos, answer, seq, self.tokenizer)
            cf_tensors_list = []
            ...
            #     print(input_ids)
            #     print('---')
            #     print(cur_rec_tensors)
            return cur_rec_tensors
        else:
            cur_rec_tensors = self._data_sample_rec_task(user_id, items_with_noise, input_ids, target_pos, answer, seq, self.tokenizer)
            return cur_rec_tensors
